...
function Dashboard() {
  ...
  const [allEmployeesFinance, setAllEmployeesFinance] = useState([])
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((r) => setAllEmployeesFinance(r.data))
  ...
  const allSalary = allEmployeesFinance.reduce(
    (p, c) => p + Number(c.baseSalary),
    0,
  )
  const allBonus = allEmployeesFinance.reduce((p, c) => p + Number(c.bonus), 0)
  const allPenalty = allEmployeesFinance.reduce(
    (p, c) => p + Number(c.penalty),
    0,
  )
  const allKpiAmount = allEmployeesFinance.reduce(
