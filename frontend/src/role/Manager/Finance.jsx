...
function Finance() {
  ...
  useEffect(() => {
    ...
    employees.forEach((employee) => {
      const employeeFinanceRecord = financeData.find(
        (finance) => finance.employeeId === employee.id
      )

      if (employeeFinanceRecord) {
        const monthlyData = employeeFinanceRecord.monthly.find(
        ...
        if (monthlyData) {
          merged.push({
            ...
            baseSalary: monthlyData.baseSalary,
            kpiAmount: monthlyData.kpiAmount,
            kpiPercent:
              monthlyData.baseSalary > 0
                ? Math.round((monthlyData.kpiAmount / monthlyData.baseSalary) * 100)
                : 0,
            bonus: monthlyData.bonus,
            penalty: monthlyData.penalty,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Find employee finance record from financeData
      const financeRecord = financeData.find(
        (finance) => finance.employeeId === editData.id
      )
      ...
      const updatedMonthly = financeRecord.monthly.map((m) => {
      ...
      setFinanceData((prev) =>
        prev.map((item) =>
          item.employeeId === editData.id ? updatedRecord : item
        )
      )
  ...
  return (
    <div className="w-full min-h-screen bg-[#0f172a] p-6 text-slate-300 font-sans">
      <div className="max-w-[1600px] mx-auto">
        ...
        {userCard && editData && (
          <>
            <div onClick={closeModal} className="fixed inset-0 bg-black/70 z-40" />
            <div className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] max-h-[90vh] overflow-y-auto bg-[#0f172a] rounded-2xl border border-white/10">
              <div className="sticky top-0 bg-[#0f172a] p-6 border-b border-white/10 flex justify-between z-10">
                <h2 className="text-xl font-bold text-cyan-400">{editData.fullName}</h2>
              ...
              <div className="p-6 space-y-4">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (f.key === 'baseSalary' || f.key === 'kpiPercent') {
                            const base =
                              f.key === 'baseSalary' ? Number(value) : Number(editData.baseSalary)
                            const percent =
                              f.key === 'kpiPercent' ? Number(value) : Number(editData.kpiPercent)
                        ...
                        className={`px-3 py-2 rounded-xl bg-slate-900 border border-white/10 text-white outline-none ${
                          f.readOnly ? 'opacity-70 cursor-not-allowed' : 'focus:border-cyan-500'
                        }`}
                      />
                    </div>
                  ))}
                ...
                <div className="bg-[#0b1221] p-4 rounded-xl border border-white/10">
                  <div className="flex justify-between items-center">
                    <span className="text-xs uppercase text-cyan-500 font-semibold">Total Salary</span>
        ...
        <div className="overflow-x-auto rounded-2xl border border-white/10 bg-[#0f172a]">
          <table className="w-full text-sm">
            <thead className="bg-[#0f172a] text-slate-400 border-b border-white/10">
              <tr>
                {columns.map((c) => (
                  <th key={c} className="px-4 py-3 text-left whitespace-nowrap font-medium">
                    {c}
                  </th>
                ))}
              </tr>
              ...
              {currentData.length > 0 ? (
                currentData.map((row) => (
                  <tr key={row.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">{row.paymentDate}</td>
                    <td className="px-4 py-3 capitalize whitespace-nowrap">{row.paymentMethod}</td>
