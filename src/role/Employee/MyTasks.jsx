...
function MyTasks() {
  ...
  return (
    <div className="p-10 bg-[#020617] min-h-screen">
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-5 mb-16 bg-slate-900/40 p-6 rounded-[2rem] border border-slate-800">
        <input
          type="text"
          placeholder="SEARCH TASKS..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="flex-1 bg-slate-950/50 border border-slate-700/50 px-6 py-4 rounded-xl text-white outline-none focus:border-cyan-500 transition-all font-bold text-xs uppercase tracking-widest"
        />

        <select
          value={sort}
          onChange={(e) => setSort(e.target.value)}
          className="bg-slate-950/50 border border-slate-700/50 px-6 py-4 rounded-xl text-cyan-400 outline-none font-black text-xs uppercase"
        >
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-8 pb-20">
        {filteredData.map((task) => (
          ...
          >
            ...
            <div className="relative bg-[#0f172a] border border-slate-800 rounded-[2rem] p-8 h-full flex flex-col hover:border-purple-500/50 hover:shadow-2xl transition-all duration-300">
              <div className="flex items-center mb-6">
                ...
                <h3 className="text-lg font-bold text-white ml-4 line-clamp-1 italic tracking-tight">
                  {task.taskName}
                </h3>
              </div>
              ...
              <div className="space-y-4 mb-8 flex-1">
                <div className="flex items-center gap-4">
                  <HiOutlineUser className="text-slate-500 text-lg" />
                  <div>
                ...
                <div className="flex items-center gap-4">
                  <HiOutlineCalendar className="text-slate-500 text-lg" />
                  <div>
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Target Deadline</p>
      ...
      {selectedTask && (
        ...
        >
          <div
            className="bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[3rem] p-10 relative animate-in zoom-in duration-300 shadow-2xl"
            onClick={e => e.stopPropagation()}
          >
            <button onClick={() => setSelectedTask(null)} className="absolute top-8 right-8 text-slate-500 hover:text-white transition">
              ✕
            </button>
            <h2 className="text-cyan-400 text-[10px] font-black tracking-[0.3em] uppercase mb-4">
              CORE TASK DETAIL
            </h2>
            <h3 className="text-2xl font-black text-white mb-10 italic tracking-tighter">
              {selectedTask.taskName}
            </h3>

            <div className="space-y-6">
              <div className="flex justify-between items-center py-4 border-b border-white/5">
                <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Assignee</span>
                <span className="text-white font-bold tracking-tight">{selectedTask.employeeName}</span>
              </div>
              <div className="flex justify-between items-center py-4 border-b border-white/5">
                <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Final Deadline</span>
