...
function MyVacations() {
  ...
  useEffect(() => {
    const fetchVacations = async () => {
      try {
        ...
        const myData = res.data.filter(v => v.employeeName === user?.fullName)
        setVacations(myData)
      } catch (err) {
        console.error(err)
      } finally {
  ...
  if (loading) return <div className="min-h-screen bg-[#020617] flex items-center justify-center text-emerald-400 font-bold animate-pulse tracking-widest">LOADING VACATION LOGS...</div>
    ...
    <div className="min-h-screen bg-[#020617] p-10 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 pb-20">
          {filtered.map((v) => (
            <div key={v.id} className="group relative bg-[#0f172a] border border-slate-800 p-8 rounded-[2.5rem] hover:border-emerald-500/50 hover:shadow-2xl transition-all duration-300">
              <div className="flex justify-between items-start mb-6">
                ...
                <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${v.status === 'approved' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'
                  }`}>
                  {v.status}
                </span>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Duration Period</p>
                ...
                <div className="pt-6 border-t border-white/5 flex items-center gap-2">
                  ...
                  <p className="text-xs text-slate-500 font-medium">Requested on {v.startDate}</p>
                </div>
              </div>
            </div>
          ))}
          ...
          {filtered.length === 0 && (
            <div className="col-span-full text-center py-32 bg-slate-900/20 border border-dashed border-slate-800 rounded-[3rem]">
              <p className="text-slate-500 font-bold uppercase tracking-[0.4em]">NO VACATION RECORDS ON FILE FOR {selectedYear}</p>
            </div>
          )}
        </div>
      </div>
