...
function MyAnnouncements() {
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center h-screen bg-[#020617] text-cyan-400 font-bold tracking-widest animate-pulse">
    ...
    <div className="min-h-screen bg-[#020617] p-10 font-sans">
      <div className="max-w-6xl mx-auto mb-16 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div>
            ...
            <p className="text-slate-400 font-bold uppercase tracking-widest text-xs opacity-60">
        ...
        <div className="mt-12 flex gap-4 bg-slate-900/40 p-3 rounded-2xl border border-slate-800">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full bg-slate-950/50 border border-slate-700/50 py-4 pl-14 pr-6 rounded-xl text-white outline-none focus:border-cyan-500 transition-all font-bold text-xs uppercase tracking-widest"
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-8 pb-20">
        {filteredAnnouncements.map((ann) => (
          ...
          >
            ...
            <div className="relative bg-[#0f172a] border border-slate-800 rounded-[2rem] p-8 h-full flex flex-col hover:border-cyan-500/50 hover:shadow-2xl transition-all duration-300">
              ...
              <div className="flex justify-between items-center pt-6 border-t border-white/5">
                ...
                <button className="text-xs font-black text-cyan-400 hover:text-white transition-colors uppercase tracking-widest">
                  READ MORE +
                </button>
              </div>
            </div>
      ...
      {!filteredAnnouncements.length && (
        <div className="max-w-6xl mx-auto py-32 text-center bg-slate-900/20 border border-dashed border-slate-800 rounded-[3rem]">
          <p className="text-slate-500 font-bold uppercase tracking-[0.4em]">NO PRESS RELEASES FOUND FOR {selectedYear}</p>
        </div>
      )}
      ...
      {selected && (
        ...
        >
          <div
            className="bg-[#0f172a] border border-white/10 w-full max-w-2xl rounded-[3rem] p-12 relative animate-in zoom-in duration-300 shadow-2xl"
            onClick={e => e.stopPropagation()}
          >
            <button onClick={() => setSelected(null)} className="absolute top-8 right-8 text-slate-500 hover:text-white transition">
              <FaTimes size={24} />
            </button>
            <div className="text-cyan-400 text-xs font-black tracking-[0.3em] uppercase mb-4 flex items-center gap-2">
              <FaBell /> PRESS RELEASE
            </div>
            <h2 className="text-3xl font-black text-white mb-8 italic tracking-tighter">
              {selected.title}
            </h2>
            <div className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-10 flex items-center gap-2">
            ...
            <div className="flex justify-end">
              <button onClick={() => setSelected(null)} className="px-8 py-3 rounded-2xl bg-slate-800 hover:bg-slate-700 text-white font-black uppercase text-[10px] tracking-widest transition-all">
                CLOSE DOCUMENT
              </button>
            </div>
          </div>
