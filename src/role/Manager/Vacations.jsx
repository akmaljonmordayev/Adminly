...
const STATUS_CONFIG = {
  pending: { dot: 'bg-[var(--bg-secondary)]mber-400 shadow-amber-400/50', badge: 'text-amber-300 border-amber-400/20 bg-gradient-to-r from-amber-500/10 to-amber-600/5' },
      ...
      <div className="relative z-10 p-4 md:p-8 max-w-[1500px] mx-auto">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 animate-fadeInUp">
          <div>
            ...
            <p className="text-[var(--text-secondary)] text-xs font-medium mt-1.5 tracking-wider uppercase">Manage employee vacation requests</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10 animate-fadeInUp stagger-1 opacity-0">
          {[
            ...
            <button key={s.key} onClick={() => setStatusFilter(s.key)} className={`relative overflow-hidden rounded-2xl p-5 border transition-all duration-500 text-left group cursor-pointer ${s.borderColor} ${statusFilter === s.key ? 'ring-1 ring-white/10 bg-[var(--bg-secondary)]' : 'bg-[var(--bg-secondary)]'}`}>
              ...
              <div className="relative flex items-center gap-3">
                ...
                <div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--text-secondary)]">{s.label}</p><p className="text-2xl font-black text-[var(--text-primary)]">{statusCounts[s.key]}</p></div>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 mb-8 animate-fadeInUp stagger-2 opacity-0">
          ...
          <div className="md:col-span-4 relative">
            <select value={sort} onChange={(e) => setSort(e.target.value)} className="w-full glass rounded-2xl py-3.5 px-5 appearance-none focus:outline-none focus:border-cyan-500/40 cursor-pointer text-[var(--text-secondary)] text-sm">
              ...
              <option className="bg-[var(--bg-secondary)]" value="a-z">Name: A-Z</option><option className="bg-[var(--bg-secondary)]" value="z-a">Name: Z-A</option>
            </select>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {processedData.map((v, i) => {
              ...
              return (
                <div key={v.id} className="group relative overflow-hidden rounded-2xl border border-cyan-500/10 hover:border-cyan-500/10 transition-all duration-500 hover:shadow-xl animate-fadeInUp opacity-0" style={{ animationDelay: `${i * 0.06}s` }}>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between gap-5">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <div>
                            <h3 className="text-lg font-bold text-[var(--text-primary)] group-hover:text-cyan-400 transition-colors">{v.employeeName}</h3>
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm text-[var(--text-secondary)] mt-3">
                          <div className="flex items-center gap-2"><FiCalendar className="text-cyan-500/50" /><span className="font-medium">{v.startDate}</span><span className="text-[var(--text-secondary)]">→</span><span className="font-medium">{v.endDate}</span></div>
                          <div className="px-3 py-1 bg-white/[0.03] rounded-lg text-[10px] font-black text-[var(--text-secondary)] border border-cyan-500/10">{days} day{days !== 1 ? 's' : ''}</div>
                        </div>
                      </div>
                      <div className="flex lg:flex-col gap-3 flex-shrink-0">
                        <select value={status} onChange={(e) => handleStatusChange(v.id, e.target.value)}
                          ...
                          <option className="bg-[var(--bg-secondary)]" value="pending">Pending</option><option className="bg-[var(--bg-secondary)]" value="approved">Approved</option><option className="bg-[var(--bg-secondary)]" value="rejected">Rejected</option>
                        </select>
          ...
          <div className="py-28 text-center border border-dashed border-cyan-500/10 rounded-[2rem] bg-[var(--bg-secondary)] animate-fadeInScale">
            ...
            <p className="text-[var(--text-secondary)] font-bold text-lg">No vacations found</p>
