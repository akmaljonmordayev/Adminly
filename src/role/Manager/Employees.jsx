...
function Employees() {
  ...
  if (loading) return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-[#020617]">
      <div className="w-14 h-14 border-2 border-slate-800 border-t-cyan-500 rounded-full animate-spin" />
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans relative">
      ...
      <div className="relative z-10 p-4 md:p-8 max-w-[1500px] mx-auto">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 animate-fadeInUp">
          <div>
            ...
            <p className="text-slate-600 text-xs font-medium mt-1.5 tracking-wider uppercase">Manage team members & compensation</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="relative group">
              ...
              <input className="glass rounded-2xl py-2.5 pl-11 pr-4 w-full md:w-72 outline-none focus:border-cyan-500/40 transition-all text-sm text-white placeholder:text-slate-700"
                placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            <button onClick={() => setAddOpen(true)}
              className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all active:scale-95 shadow-lg shadow-cyan-500/20 hover:shadow-xl hover:shadow-cyan-500/30">
              <FiUserPlus /> Add
            </button>
          </div>
        </header>
        ...
        {/* Table Container */}
        <div className="relative overflow-hidden rounded-[2rem] border border-white/[0.04] animate-fadeInUp stagger-1 opacity-0">
              ...
              <thead className="border-b border-white/[0.04]">
                <tr>
                  {['Employee', 'ID', 'Salary', 'Joined', 'Status', ''].map((h) => (
                    <th key={h} className={`p-5 text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] ${h === '' ? 'text-right' : ''}`}>{h}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  <tr key={u.id} className="group hover:bg-white/[0.02] transition-all duration-300 animate-fadeInUp opacity-0" style={{ animationDelay: `${i * 0.04}s` }}>
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold text-white text-sm group-hover:text-cyan-400 transition-colors">{u.fullName}</div>
                          <div className="text-[10px] text-slate-600 font-medium">{u.email}</div>
                        </div>
                      </div>
                    </td>
                    <td className="p-5">
                      <span className="text-[10px] font-mono font-bold text-slate-500 bg-white/[0.03] px-2.5 py-1 rounded-lg border border-white/[0.04]">#{u.userId}</span>
                    </td>
                    <td className="p-5">
                      <span className="text-sm font-bold text-white">{getEmployeeSalary(u.id).toLocaleString()}</span>
                    </td>
                    <td className="p-5 text-sm text-slate-500 font-medium">{u.hireDate}</td>
                    <td className="p-5">
                    ...
                    <td className="p-5 text-right">
                      <div className="flex justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => { setEditUser(u); setEditOpen(true) }} className="w-8 h-8 bg-blue-500/10 text-blue-400 rounded-lg flex items-center justify-center hover:bg-blue-500 hover:text-white transition-all"><FiEdit2 size={13} /></button>
                        <button onClick={() => handleDelete(u)} className="w-8 h-8 bg-red-500/[0.06] text-red-400/70 rounded-lg flex items-center justify-center hover:bg-red-500 hover:text-white transition-all"><FiTrash2 size={13} /></button>
                      </div>
                    </td>
                  </tr>
                )) : (
                  <tr><td colSpan="6" className="p-16 text-center">
                    <div className="w-16 h-16 mx-auto mb-5 rounded-2xl bg-white/[0.02] border border-white/5 flex items-center justify-center"><FiUser className="text-2xl text-slate-700" /></div>
                    <p className="text-slate-600 font-bold text-lg">No employees found</p>
                  </td></tr>
                )}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeInScale">
          <div className="glass-strong w-full max-w-md rounded-[2rem] shadow-2xl">
            <div className="p-6 border-b border-white/[0.04] flex justify-between items-center">
              <h2 className="text-lg font-bold text-white">{addOpen ? 'New Employee' : 'Edit Employee'}</h2>
              <button onClick={() => { setAddOpen(false); setEditOpen(false) }} className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center text-slate-500 hover:text-white hover:bg-white/10 transition-all"><FiX size={16} /></button>
            </div>
              ...
              {addOpen ? (
                ...
                <>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] ml-1">Status</label>
                    <select className="w-full bg-[#060d1f] border border-white/[0.06] rounded-xl p-2.5 outline-none focus:border-cyan-500/40 text-sm text-white" value={editUser.status} onChange={(e) => setEditUser({ ...editUser, status: e.target.value })}>
            ...
            <div className="p-6 flex gap-3">
              <button onClick={() => { setAddOpen(false); setEditOpen(false) }} className="flex-1 px-4 py-2.5 rounded-xl font-bold text-slate-500 hover:bg-white/5 transition-all text-sm">Cancel</button>
              <button onClick={addOpen ? handleAdd : handleEdit} className="flex-1 px-4 py-2.5 rounded-xl font-bold bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-lg shadow-cyan-500/20 hover:shadow-xl hover:shadow-cyan-500/30 active:scale-[0.98] transition-all text-sm">
                {addOpen ? 'Create' : 'Save'}
              </button>
            </div>
          </div>
      ...
      <ToastContainer theme="dark" position="bottom-right" autoClose={2000} toastClassName="!bg-[#0a1128] !border !border-white/10 !rounded-2xl !shadow-2xl" />
    </div>
  )
}
...
const ModalInput = ({ label, icon, value, onChange, type = 'text' }) => (
  <div className="space-y-1.5">
    ...
    <div className="relative">
      ...
      <input type={type} className="w-full bg-[#060d1f] border border-white/[0.06] rounded-xl py-2.5 pl-11 pr-4 outline-none focus:border-cyan-500/40 transition-all text-sm text-white" value={value} onChange={(e) => onChange(e.target.value)} />
    </div>
  </div>
)
