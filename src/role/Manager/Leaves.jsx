...
const STATUS_CONFIG = {
  pending: { dot: 'bg-amber-400 shadow-amber-400/50', badge: 'text-amber-300 border-amber-400/20 bg-gradient-to-r from-amber-500/10 to-amber-600/5', cardBorder: 'border-l-amber-500/60' },
      ...
      <div className="relative z-10 p-4 md:p-8 max-w-[1500px] mx-auto">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 animate-fadeInUp">
          <div>
            ...
            <p className="text-slate-600 text-xs font-medium mt-1.5 tracking-wider uppercase">Manage leave & resignation requests</p>
          </div>
          <div className="flex items-center gap-2 glass rounded-2xl p-1.5">
            <button onClick={() => { setSelectedYear('all'); setCurrentPage(1) }} className={`px-5 py-2 rounded-xl text-sm font-bold transition-all duration-300 ${selectedYear === 'all' ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-lg shadow-cyan-500/25' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}>All</button>
            {availableYears.map((y) => (
              <button key={y} onClick={() => { setSelectedYear(y); setCurrentPage(1) }} className={`px-5 py-2 rounded-xl text-sm font-bold transition-all duration-300 ${selectedYear === y ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-lg shadow-cyan-500/25' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}>{y}</button>
            ))}
          </div>
        </header>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {currentData.map((c, i) => {
              ...
              return (
                <div key={id} className={`group relative overflow-hidden rounded-2xl border-l-[3px] ${config.cardBorder} border border-white/[0.04] hover:border-white/10 transition-all duration-500 hover:shadow-xl animate-fadeInUp opacity-0`} style={{ animationDelay: `${i * 0.06}s` }}>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between gap-5">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-lg font-bold text-white group-hover:text-cyan-400 transition-colors">{c.employeeName || 'Unknown'}</h3>
                        ...
                        <p className="text-slate-500 text-sm mb-3">{c.description || 'No description'}</p>
                        <div className="flex items-center gap-2 text-xs text-slate-600"><FiCalendar className="text-cyan-500/50" /><span className="font-medium">Applied: {c.appliedDate || 'N/A'}</span></div>
                      </div>
                      <div className="flex lg:flex-col gap-3 flex-shrink-0">
                        <select value={status} onChange={(e) => handleStatusChange(id, e.target.value)}
                          ...
                          <option className="bg-[#060d1f]" value="approved">Approved</option>
                          <option className="bg-[#060d1f]" value="rejected">Rejected</option>
                        </select>
          ...
          <div className="py-28 text-center border border-dashed border-white/[0.06] rounded-[2rem] bg-[#060d1f] animate-fadeInScale">
            ...
            <p className="text-slate-600 font-bold text-lg">No leave requests found</p>
